...
def test_marker_grammar_covers_all_text_families():
    """The corpus carries ~19 marker shapes, not just bhp's 3-group lowercase.
    The verifier MUST extract every family or it falsely flags whole texts as
    hallucinated. (Discovered 2026-06-23: the bhp-only regex returned ZERO
    markers for Gheranda/RV/Manu/etc → a phantom 100% hallucination rate.)
    """
    # one literal marker per real family seen across data/chunks/*.jsonl
    window = ("... bhp_01.03.040 ... GhS_1.0 ... GhS_1.44 ... RV_1 ... gbr_1 ... "
              "BrP_1.1 ... Manu_1.1 ... SvetUp_1.1 ... KB_1 ... R_1 ... ps_1 ... "
              "isup_1 ... bg_01.15 ... mbh_1.2.3 ...")
    found = set(verify.extract_markers(window))
    for m in ["bhp_01.03.040", "GhS_1.0", "GhS_1.44", "RV_1", "gbr_1", "BrP_1.1",
              "Manu_1.1", "SvetUp_1.1", "KB_1", "R_1", "ps_1", "isup_1",
              "bg_01.15", "mbh_1.2.3"]:
        assert m in found, f"marker family not extracted: {m} — verifier blind to its text"


def test_range_cite_grounds_on_its_start_marker():
    """Decoders cite ranges like 'GhS_1.44-GhS_1.60'; the window holds only the
    individual start marker. _normalize_cite must reduce the range to GhS_1.44 so
    it grounds against the window — else every ranged citation false-flags."""
    present = {"GhS_1.44"}
    node = {"name": "Shatkarma", "verse_ranges": ["GhS_1.44-GhS_1.60"]}
    res = verify.check_node(node, present)
    assert res["grounded"] is True, "range cite should ground on its start marker"


def test_uppercase_and_single_group_markers_ground():
    """Gheranda's actual format (uppercase prefix, single dot-group) must ground
    — the exact case the bhp-only regex got wrong."""
    present = set(verify.extract_markers("text with GhS_1.0 and GhS_2.1 markers"))
    node = {"name": "Gheranda", "verse_ranges": ["GhS_1.0"]}
    assert verify.check_node(node, present)["grounded"] is True


def test_marker_regex_does_not_over_match_ordinary_underscores():
    """Guard: the looser grammar must NOT treat ordinary snake_case words as
    verse markers, or window marker-sets get polluted and false-ground noise."""
    found = set(verify.extract_markers(
        "chapter_1 section_2 verse_5_text foo_bar self_test"))
    assert found == set(), f"over-matched non-markers: {found}"
