...
# Canonical verse markers embedded in the GRETIL text. The corpus uses ~19
# distinct shapes across its 49 texts, NOT just Bhagavata's 3-group lowercase:
#   bhp_01.03.040 (3 groups), bg_01.15 / GhS_1.0 / Manu_1.1 (1 dot, mixed case),
#   RV_1 / gbr_1 / KB_1 (no dots at all). An earlier bhp-only regex
#   (`[a-z]{2,5}_\d+(?:\.\d+)+`) extracted ZERO markers for every non-Bhagavata
#   text → a phantom 100% hallucination rate (fixed 2026-06-23). Grammar:
#   <1-6 letters, any case>_<digits>(.<digits>)*  — the trailing groups optional,
#   so a bare `RV_1` matches. The leading `\b` + required `\d` after `_` keeps
#   ordinary snake_case words (chapter_1, foo_bar) from matching.
