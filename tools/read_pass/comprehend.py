...
Rules:
- Use the verse markers in the text (e.g. bhp_01.01.001, mbh_06.023.001) to fill verse_ranges.
...
# in-house decode (CLAUDE is the LLM, via a Workflow fan-out) — resolves each
# chapter's answer from a disk cache; no network. See inhouse.py.
from tools.read_pass.inhouse import make_inhouse_caller as _make_inhouse_caller

_CALLERS = {"deepseek": _call_deepseek, "gemini": _call_gemini,
            "inhouse": _make_inhouse_caller()}
...
def resolve_provider() -> tuple:
    ...
    INHOUSE_DECODE=1 wins over any LLM key — it routes comprehension to our own
    compute (Claude subagents) so we never burn the Gemini free tier again."""
    if os.environ.get("INHOUSE_DECODE", "") in ("1", "true", "yes"):
        return "inhouse", "inhouse", _MODELS["inhouse"]
...
def comprehend_window(window: Dict[str, Any],
    ...
    # Providers without server-side schema enforcement need it embedded in the prompt.
    # deepseek: no schema param. inhouse: the Claude subagent must see the exact target
    # shape (and the prompt hash IS the cache key, so this must be deterministic).
    if provider in ("deepseek", "inhouse") and not caller:
