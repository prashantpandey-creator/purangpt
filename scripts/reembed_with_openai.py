...
print("Setting up local database schema for 1536-d OpenAI vectors...")
cur.execute("CREATE EXTENSION IF NOT EXISTS vector;")

# Drop old table if exists to change vector dimension
...
print("Starting migration and OpenAI re-embedding...")
while offset < total_count:
    res = supabase.table(TABLE_NAME).select("id, content, metadata").range(offset, offset + BATCH_SIZE - 1).execute()
    data = res.data
    ...
    # text-embedding-3-small allows large batches.
    print(f"Generating OpenAI embeddings for {len(texts)} verses...")
    try:
