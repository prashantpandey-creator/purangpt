...
def test_kin_direction(mem: Memory):
    """Kin edges must be read by POSITION (src=parent), not the noisy rel word.
    Regression guard for the 'Dasharatha (son)' inversion bug."""
    print("\n--- character kin directionality ---")
    d = character.character_sheet("Rama", mem)["data"]
    fam = d["family"]
    ...
    def rel_of(*names):
        ...
        for f in fam:
            ...
            if any(nm.lower() in fn for nm in names):
                out.append((f.get("relationship") or "").lower())
        return out

    dasa = rel_of("Dasharatha", "Daśaratha", "Daśaratha")
    _check("Dasharatha is Rama's father, never son",
           "father" in dasa and "son" not in dasa, f"got {dasa}")
    mothers = rel_of("Kausalya", "Kaushalya", "Kaikeyi", "Sumitra")
    _check("Rama's parents never labeled son/daughter",
           mothers and all(r not in ("son", "daughter") for r in mothers),
           f"got {mothers}")
    sita = rel_of("Sita")
    _check("Sita is Rama's wife, never husband",
           "husband" not in sita and any(r in ("wife", "spouse") for r in sita),
           f"got {sita}")
    kids = rel_of("Lava", "Kusha", "Kuśa")
    _check("Rama's sons never labeled father/mother",
           kids and all(r not in ("father", "mother") for r in kids),
           f"got {kids}")
    # the core directionality invariant: no single person is labeled both a
    # senior (parent/ancestor) AND a junior (child/descendant) of Rama.
    # (alias variants like Dasharatha/Daśaratha staying separate is a distinct
    # entity-merge problem, not a directionality one.)
    by_name = {}
    for f in fam:
        by_name.setdefault((f.get("name") or "").lower(), set()).add(
            (f.get("relationship") or "").lower())
    senior = {"father", "mother", "parent", "grandfather", "grandmother",
              "grandparent", "ancestor"}
    junior = {"son", "daughter", "child", "grandson", "granddaughter",
              "grandchild", "descendant"}
    contradictory = {nm for nm, rels in by_name.items()
                     if (rels & senior) and (rels & junior)}
    _check("no person is both senior and junior to Rama",
